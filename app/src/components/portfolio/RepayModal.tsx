...
interface RepayModalProps {
  ...
  onClose: () => void;
  /** future: pass onRepay(amount: number) => Promise<void> */
  onRepay?: (amount: number) => Promise<void>;
}

export function RepayModal({ position, onClose, onRepay }: RepayModalProps) {
  const [amount, setAmount] = useState("");

  const numAmount = parseFloat(amount) || 0;
  const maxRepay = position.debtAmount;
  const remaining = Math.max(maxRepay - numAmount, 0);
  const repayPct = maxRepay > 0 ? Math.min(numAmount / maxRepay, 1) : 0;

  // Health factor improves as debt decreases (linear approximation)
  const projectedHF =
    repayPct >= 1 ? 99 : position.healthFactor / (1 - repayPct * 0.6);
          ...
          <div className="rounded-xl border border-[#c698e5]/10 divide-y divide-[#c698e5]/8">
            ...
            <div className="flex items-center justify-between px-3.5 py-2.5">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-xs tabular-nums text-[#efe0f7]/40 line-through">
                ...
                >
                  {projectedHF >= 99 ? "∞" : projectedHF.toFixed(2)}
                </span>
              </div>
            </div>
          </div>
